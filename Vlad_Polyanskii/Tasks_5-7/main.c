...
void print_str(table* t, int line){
    ...
    lseek(t->fd, offset * sizeof(char), SEEK_SET);

    char *buff = (char*)malloc(sizeof(char));
    int len = t->matrix[line].str_len;
    read(t->fd, buff, sizeof(char) * (len - 1));
    printf("%s\n", buff);
}

void print_file(table* t){
    char *buff = (char*)malloc(sizeof(char));
    read(t->fd, buff, sizeof(char) * t->flen);
    printf("FILE:\n%s\n", buff);
}
